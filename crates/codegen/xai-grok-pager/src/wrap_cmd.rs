//! `echo-build wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `echo-build ssh` wrapper: spawns an arbitrary command inside a
...
//! run — a single shell-quoted string (`echo-build wrap "mycli ssh host"`) or a shell
...
/// Run the `echo-build wrap` command.
///
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("echo-build wrap: no command given"))?;
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!(
                    "echo-build wrap: wrapped mode failed, running without PTY wrapping: {e}"
                );
...
/// The program and argv `echo-build wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`echo-build wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`echo-build wrap "$PROG" ...` with `$PROG` unset)
...
///
/// Unlike `echo-build ssh`, `echo-build wrap` does not gate on the terminal brand: the user
