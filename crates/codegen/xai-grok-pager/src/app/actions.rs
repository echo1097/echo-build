...
pub enum Action {
    ...
    OpenTranscriptPager,
    /// Minimal mode (`echo-build --minimal`): re-print the most-recently committed
    ...
    /// User submitted an OpenRouter API key for secure storage.
    SubmitApiKey(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `echo-build dashboard`).
    OpenDashboard,
...
pub enum Effect {
    ...
    /// Store an OpenRouter API key, then authenticate through the API-key path.
    SaveApiKey { request_seq: u64, api_key: String },
