#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `echo-build logout` /
    /// `echo-build login` guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          run `echo-build logout` then `echo-build login`"
                    .into(),
            },
            &mut session,
            &mut scrollback, false);
