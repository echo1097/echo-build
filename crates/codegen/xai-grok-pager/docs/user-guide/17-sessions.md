...
Run `echo-build --resume` without an ID to resume the most recent session for the current directory.

### From the Welcome Screen

When you launch `echo-build`, the welcome screen lists recent sessions for the current directory. Select one to resume it.
...
echo-build -p "Continue where we left off" -r <session-id>
...
## The echo-build sessions Subcommand

List or search sessions from the command line. `echo-build sessions` requires a subcommand:

```bash
# List recent sessions for the current directory
echo-build sessions list

# Limit the number of results (default 20)
echo-build sessions list --limit 50

# Search sessions by keyword (matches titles and prompts)
echo-build sessions search "rate limit"
```

`echo-build sessions list` shows sessions for the current working directory, grouped by worktree label. Each row lists the session ID, the creation and update dates, the source status, and the summary. `echo-build sessions search` combines a local SQLite index with remote results.
...
Resume a session in a fresh worktree with `echo-build -w -r <session-id>`.
...
The smaller state files -- `summary.json`, `plan.json`, and `signals.json` -- are plain JSON rather than JSONL. JSONL is the source of truth for session content; `echo-build sessions search` additionally maintains a local SQLite FTS5 index over session titles and prompts for fast keyword search.
