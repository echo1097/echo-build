...
| `--plugin-dir` (CLI flag, `echo-build agent`) | Process -- loaded for that agent process only | Trusted automatically |
...
```bash
echo-build plugin list [--json] [--available]   # List installed plugins (--available requires --json)
echo-build plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
echo-build plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
echo-build plugin update [<name>]               # Omit the name to update all plugins
echo-build plugin enable <name>
echo-build plugin disable <name>
echo-build plugin details <name>                # Show the plugin's component inventory
echo-build plugin validate [<path>]             # Validate plugin.json (default: current directory)
echo-build plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `echo-build plugin install <source>` without `--trust` and Grok prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
```bash
echo-build plugin marketplace list [--json]
echo-build plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
echo-build plugin marketplace remove <url>      # Git URL or local path of a configured source
echo-build plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
echo-build plugin marketplace add my-org/team-plugins
echo-build plugin marketplace list
echo-build plugin install my-org/team-plugins --trust
echo-build plugin list
echo-build plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `echo-build plugin` CLI.
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `echo-build plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.
...
```bash
echo-build plugin install <source> --trust
```
...
Run `echo-build inspect` to see every discovered plugin and what it provides:

```bash
echo-build inspect          # Show plugins with their skills, agents, hooks, and MCP servers
echo-build inspect --json   # Emit machine-readable JSON
```
