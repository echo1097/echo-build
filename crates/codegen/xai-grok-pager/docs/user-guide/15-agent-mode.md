...
Agent mode runs Grok as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`echo-build -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `echo-build agent` command and apply to every mode. Pass them before the mode name, for example `echo-build agent --model grok-build stdio`. The `stdio` subcommand itself takes no options.
...
```bash
echo-build agent serve --bind 127.0.0.1:2419 --secret <token>
```
