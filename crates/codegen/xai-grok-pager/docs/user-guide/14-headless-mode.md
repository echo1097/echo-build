...
By default, each `echo-build -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
echo-build -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
echo-build -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
echo-build -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
echo-build -p "What's the secret number?" --resume abc123
```
...
echo-build -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
echo-build --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  echo-build -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
RESULT=$(echo-build -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
echo-build -p "Format all files" --yolo

# Run tests and fix failures
echo-build -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
echo-build -p "Run the test suite" --yolo
```
...
- **`echo-build login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`echo-build login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `echo-build -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug echo-build -p "..." 2> debug.log`.
...
- Resume: `echo-build -p "continue" --resume "<id>"` or `echo-build -p "continue" --continue`
